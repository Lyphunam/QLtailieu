	                ...
	                <div class="col-12" style="width: 100%;">
			                    ...
			                    <div class="card-body">
			                      <h4 class="card-title">UPLOAD, UPDATE bài giảng</h4>
			                        <div class="form-group">
			                          ...
			                          <select class="form-control form-control-lg" id="mamon" name="mamon">
			                            <?php
			                                for ($i=0; $i < $count; $i++) { 
			                            ?>
			                        ...
			                        <div class="form-group">
			                          ...
			                          <select class="form-control form-control-lg" id="maloai" name="maloai">
			                            <?php
			                    ...
			                    <div class="card-body">
			                      <h4 class="card-title"></h4>
			                        <div class="form-group">
			                          ...
			                          <input type="text" class="form-control" id="lienket" name="lienket" placeholder="Nhập liên kết bài học" required/>
			                        </div>
			                        <div class="form-group">
			                          <label for="exampleFormControlSelect1">Đặt tên bài học</label>
			                          <input type="hidden" name="mabaihocEdit" id="mabaihocEdit">
			                          <input type="text" class="form-control" id="tenbaihoc" name="tenbaihoc" placeholder="Nhập tên bài học" required/>
			                        </div>
			                        <button style="visibility: visible;" type="submit" id="uploadbaigiang" name="uploadbaigiang" class="btn btn-success mr-2">UPLOAD</button>
			                        <button style="visibility: hidden;" type="submit" id="updatebaigiang" name="updatebaigiang" class="btn btn-success mr-2">UPDATE</button>
			                    </div>
			                  </div>
	                	</div>
			            </form>
			            ...
			            <?php
			            	...
			            	if (isset($_POST['uploadbaigiang'])) {
			            		...
			            		$nguoidang = $_SESSION['tengiaovien'];
			            		if($db->AddDataToTableBaiHoc($mamonhoc,$malbg,$mabaihoc,$tenbaihoc,'',$lienket,$nguoidang) == true){
			            	...
			            	// Update bài giảng
			            	if (isset($_POST['updatebaigiang'])) {
			            		$mamonhoc = $_POST['mamon'];
			            		$malbg = $_POST['maloai'];
			            		$mabaihoc = $_POST['mabaihocEdit'];
			            		$tenbaihoc = $_POST['tenbaihoc'];
			            		$lienket = $_POST['lienket'];
			            		if($db->UpdateDataTableBaiHoc($mamonhoc,$malbg,$mabaihoc,$tenbaihoc,$lienket) == 1){
			            	...
			            	if (isset($_POST['deletebaigiang'])) {
			            		...
			            		if($db->DeleteDataTableBaiHoc($mabaihoc) == true){
	                ...
	                <div class="col-12">
	                  <div class="card">
	                    <div class="card-body">
	                      ...
	                      <!-- <form method="post"> -->
	                      	<table class="table-bordered" id="tableBaiGiang">
				                <thead>
				                    <tr>
				                ...
				                <tbody>
				                	<?php
				                		$DataByNameGV = $db->GetAllBaiHocByTenGV($_SESSION['tengiaovien']);
				                		if ($DataByNameGV != null) {
				                		
				                		for($i=0;$i<count($DataByNameGV);$i++){
				                			$tbh = $DataByNameGV[$i]['TENBAIHOC'];
				                			$malbg = $DataByNameGV[$i]['MALOAIBG'];
				                			$dataten = $db->GetNameMaLBG($malbg);
				                			$tenlbg = $dataten['TENLOAIBG'];
				                			$mamh = $DataByNameGV[$i]['MAMONHOC'];
				                			$datatenmh = $db->GetNameMonHoc($mamh);
				                			$tenmh = $datatenmh['TENMONHOC'];
				                			$link = $DataByNameGV[$i]['LIENKETTAILIEU'];
				                	?>
				                    <tr>
				                        <td><?php echo $tbh;?></td>
				                        <td><?php echo $tenlbg;?></td>
				                        <td><?php echo $tenmh;?></td>
				                        <td style="display: none;"><?php echo $DataByNameGV[$i]['MABAIHOC'];?></td>
				                        <td style="display: none;"><?php echo $link;?></td>
				                        <td style="display: none;"><?php echo $mamh;?></td>
				                        <td style="display: none;"><?php echo $malbg;?></td>
				                        <td id="EditRow">
				                        	<input type="hidden" name="<?php echo $DataByNameGV[$i]['MABAIHOC'];?>" value="<?php echo $DataByNameGV[$i]['MABAIHOC'];?>">
